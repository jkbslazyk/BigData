...
val tempF = tempE
  .select( $"*", year($"xxxx"), month($"xxxx"), dayofyear($"xxxx") )
  .withColumnRenamed("year(xxxx)", "year")
  .withColumnRenamed("month(xxxx)", "month")
  .withColumnRenamed("dayofyear(xxxx)", "day of year")
tempF.show()         

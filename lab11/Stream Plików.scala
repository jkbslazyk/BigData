...
for(i<-0 to 40){ dbutils.fs.cp(s"databricks-datasets/structured-streaming/events/file-$i.json", s"dbfs:/FileStore/streamDir/file-$i.json") }
...
// MAGIC %sql select action, date_format(window.end, "MMM-dd HH:mm") as time, count from qr order by time, action
...
// MAGIC select action, date_format(window.end, "MMM-dd HH:mm") as time, count from qr order by time, action

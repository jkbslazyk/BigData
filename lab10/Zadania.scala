...
val par = df.repartition(100)

// COMMAND ----------

par.write.format("delta").partitionBy("Country").mode("overwrite").save("dbfs:/FileStore/tables/online_retail")
...
spark.sql(s"""
  CREATE TABLE retail
  USING Delta
  LOCATION 'dbfs:/FileStore/tables/online_retail'
""")
...
// TODO
val plikiPrzed = dbutils.fs.ls("dbfs:/FileStore/tables/online_retail/Country=Sweden").length
...
// TODO
val plikiPo = dbutils.fs.ls("dbfs:/FileStore/tables/online_retail/Country=Sweden").length
